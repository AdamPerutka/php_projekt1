...
function userExist($db, $login, $email) {
    ...
    $sql = "SELECT id FROM users3 WHERE login = :login OR email = :email";
    $stmt = $db->prepare($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strlen($_POST['password']) < 8) {
        $errmsg .= "Heslo musi obsahovat aspon 8 znakov. ";
    }
    //TODO: odkomentovat potom
    // if (!preg_match("#[0-9]+#", $_POST['password'])) {
    //     $errmsg .= "Heslo musi obsahovat aspon jedno cislo. ";
    // }
    // if (!preg_match("#[A-Z]+#", $_POST['password'])) {
    //     $errmsg .= "Heslo musi obsahovat aspon jedno velke pismeno. ";
    // }
    // if (!preg_match("#[a-z]+#", $_POST['password'])) {
    //     $errmsg .= "Heslo musi obsahovat aspon jedno male pismeno. ";
    // }

    // Validácia mena
    if (!preg_match("/^[a-zA-ZáäčďéěíĺľňóôŕšťúýžÁÄČĎÉĚÍĹĽŇÓÔŔŠŤÚÝŽ'\-\s]+$/u", $_POST['firstname'])) {
        $errmsg .= "Meno môže obsahovať len písmená, medzery, apostrofy a pomlčky.<br>";
    }

    // Validácia priezviska
    if (!preg_match("/^[a-zA-ZáäčďéěíĺľňóôŕšťúýžÁÄČĎÉĚÍĹĽŇÓÔŔŠŤÚÝŽ'\-\s]+$/u", $_POST['lastname'])) {
        $errmsg .= "Priezvisko môže obsahovať len písmená, medzery, apostrofy a pomlčky.<br>";
    }



    if (empty($errmsg)) {
        $sql = "INSERT INTO users3 (fullname, login, email, password, 2fa_code) VALUES (:fullname, :login, :email, :password, :2fa_code)";
...
<head>
    ...
    <style>
        ...
        /* Minimalistic and centered design */
        .code-wrapper {
            display: flex;
